...
- **客户区**（中）：**TileMapViewport** — Canvas 多层绘制；**viewport-inner** 承担四周留白与指针/滚轮命中；**viewport-ui** 叠层（不随中键滚动）：左上 **CanvasToolsBar**（标题「工具栏」+ 选择/填充/橡皮 + 清空地图 + 适应/1:1，图标+短标签）；**选择**支持点选、Ctrl 多选、拖框、Shift+拖框追加；**填充**对当前选区批量铺块，无选区时仅填充单击格；右上 **CanvasDisplayPanel**（标题「显示」+ 三个 **checkbox**：网格/原点/碰撞体积）。
