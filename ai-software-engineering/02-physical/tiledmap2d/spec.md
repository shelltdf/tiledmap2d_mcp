...
| `kind` | string | 否 | `tile`（瓦片层）、`image`（精灵/Sprite 占位叠色）、`area`（区域/Location 层，与瓦片层同属可绘制可拾取逻辑）；缺省为 `tile` |
...
主窗口为 **SDI** 单页壳：**菜单栏** → **WinToolbar** → **主工作区** → **状态栏**（见概念层 UI 说明）。全局视觉由 `tiledmap2d/src/styles/win-theme.css` 提供 **Windows 10/11 桌面近似**；**浅色**为默认 `:root` 变量集，**深色**由 `html[data-theme="dark"]` 覆盖（**主题**菜单：**跟随系统** / **浅色** / **深色**，见 `useAppTheme.js`）。界面文案支持 **中文 / 英文**（`tiledmap2d/src/i18n/messages.js` + `useAppI18n`）。

- **菜单栏**：**文件**（新建/打开/保存/另存为/导出与导入 TMX）、**编辑**（清空地图）、**语言**（中文 / English）、**主题**（跟随系统、浅色、深色）、**帮助**（支持的文件格式）。
...
- **地图层 Dock**（左）：可 **折叠** 为左缘竖条「地图层」按钮（点击展开）；展开时 **添加**（`AddLayerDialog`：层类型 **瓦片 / 精灵（image）/ 区域（area，界面显示 Location）**，插入 **当前之上/之下/最上/最下**）、**改名 / 删除**（当前层；至少保留一层）；列表 **拖拽** 排序；行显类型徽标、层名、显隐；点击行切换当前层（**瓦片层与区域层**可绘制/拾取；**图片层**为叠色占位）。
- **客户区**（中）：**TileMapViewport** — Canvas 多层绘制；**viewport-inner** 用 **CSS `translate`（panX/panY）** 平移画布内容，承担四周留白与指针/滚轮命中；**viewport-ui** 叠层（不随平移滚动）：左上 **CanvasToolsBar**（标题「工具栏」+ 选择/**吸取**/填充/橡皮 + 清空地图 + 适应/1:1，图标+短标签）；**选择**仅维护选区高亮，不修改块库当前块；**吸取**从当前编辑层格读取 tile id 写回块库选中；**填充 / 橡皮**：无选区时 **单击** 或 **拖动** 连续作用于经过的格（线段补全）；**有选区**时须 **点击落在选区内** 才对整段选区执行铺/擦，点在选区外无效；右上 **CanvasDisplayPanel**（标题「显示」+ 三个 **checkbox**：网格/原点/碰撞体积）。
- **平移 / 缩放**：中键拖拽更新 **panX/panY**；滚轮缩放以指针为锚点；**空格**按住临时选择工具（与 **Alt** 临时吸取互斥）；**Alt** 按住临时吸取工具（松手还原；非输入框焦点时）；方向键平移（画布聚焦）；选择工具 **右键** 清空选区。
- **状态栏**：展示就绪/错误摘要、光标格、缩放；**点击状态栏** 打开 **Log 窗口**（`LogDialog.vue`），展示提示/警告/错误历史及 **复制全部**（纯文本）。
...
- 导入 JSON / TMX 解析失败或校验失败：状态栏展示简短错误信息；文案随当前界面语言（`te` / `appLocale`）变化，并写入 **Log**（`useAppLog`）。
