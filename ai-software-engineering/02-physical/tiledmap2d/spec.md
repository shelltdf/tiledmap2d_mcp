...
- **客户区**（中）：**TileMapViewport** — Canvas 多层绘制；**viewport-inner** 承担四周留白与指针/滚轮命中；**viewport-ui** 叠层（不随中键滚动）：左上 **CanvasToolsBar**（标题「工具栏」+ 选择/画笔/橡皮 + 适应/居中，图标+短标签）、右上 **CanvasDisplayPanel**（标题「显示」+ 三个 **checkbox**：网格/原点/参考）。
